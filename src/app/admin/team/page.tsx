...
>> = {
  leads: {
    label: "Spreadsheets",
    description: "Spreadsheets, custom sheets, folders, and records",
    icon: (
...
function MemberPermissionsCell({
  ...
  const calculatePosition = useCallback(() => {
    ...
    const isUp = spaceBelow < 280 && spaceAbove > 280;
    ...
    if (isUp) {
      ...
      style.maxHeight = `${Math.min(320, Math.max(140, rect.top - 16))}px`;
    } else {
      style.top = `${rect.bottom + 6}px`;
      style.maxHeight = `${Math.min(320, Math.max(140, window.innerHeight - rect.bottom - 16))}px`;
    }
  ...
  const maxVisible = 2;
  const visibleSections = grantedSections.slice(0, maxVisible);
  const remainingCount = grantedSections.length - maxVisible;
    ...
    <div className="flex items-center gap-1.5 whitespace-nowrap overflow-visible">
      {visibleSections.map((item) => {
        ...
        return (
          <span
            key={item.key}
            className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-medium border shrink-0 shadow-2xs whitespace-nowrap ${
              isWrite
                ? "bg-emerald-50/90 dark:bg-emerald-950/40 border-emerald-200/80 dark:border-emerald-800/60 text-emerald-800 dark:text-emerald-300"
                : "bg-blue-50/90 dark:bg-blue-950/40 border-blue-200/80 dark:border-blue-800/60 text-blue-800 dark:text-blue-300"
            }`}
          >
            <span className="truncate max-w-[120px]">{config?.label || item.key}</span>
            <span
              className={`text-[9px] px-1.5 py-0.5 rounded font-bold uppercase tracking-wider leading-none shrink-0 ${
                isWrite
      ...
      {remainingCount > 0 && (
        <>
          <button
            ...
            className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs font-semibold border transition shrink-0 cursor-pointer shadow-2xs whitespace-nowrap ${
              open
                ? "bg-blue-50 dark:bg-blue-950/60 text-blue-700 dark:text-blue-300 border-blue-300 dark:border-blue-700 ring-2 ring-blue-500/20"
                : "bg-zinc-100 hover:bg-zinc-200 dark:bg-zinc-800 dark:hover:bg-zinc-700 text-zinc-700 dark:text-zinc-300 border-zinc-200 dark:border-zinc-700/70"
            }`}
          >
            <span>+{remainingCount} more</span>
            <svg
              className={`w-3.5 h-3.5 text-zinc-400 dark:text-zinc-400 transition-transform duration-150 ${
          ...
          {open &&
            ...
            createPortal(
              <div
                ref={menuRef}
                style={menuStyle}
                className="bg-white dark:bg-zinc-900 rounded-xl shadow-2xl border border-zinc-200 dark:border-zinc-800 overflow-hidden flex flex-col animate-in fade-in zoom-in-95 duration-150"
              >
                {/* Header */}
                <div className="px-3.5 py-2.5 bg-zinc-50/80 dark:bg-zinc-800/60 border-b border-zinc-200/80 dark:border-zinc-800 flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <span className="text-xs font-bold text-zinc-900 dark:text-white">Assigned Permissions</span>
                    <span className="text-[10px] font-bold px-1.5 py-0.5 rounded-full bg-blue-100 dark:bg-blue-950/80 text-blue-700 dark:text-blue-300 border border-blue-200 dark:border-blue-800/60">
                      {grantedSections.length} active
                    </span>
                  </div>
                  <button
                    type="button"
                    onClick={() => setOpen(false)}
                    className="text-zinc-400 hover:text-zinc-600 dark:hover:text-zinc-200 p-0.5 rounded-md hover:bg-zinc-200 dark:hover:bg-zinc-700 transition cursor-pointer"
                  >
                ...
                {/* Scrollable list */}
                <div className="p-1.5 max-h-56 overflow-y-auto divide-y divide-zinc-100 dark:divide-zinc-800/60 space-y-0.5">
                  {grantedSections.map((item) => {
                    const config = PERMISSION_CONFIG[item.key];
                    const isWrite = item.level === "write";
                    return (
                      <div
                        key={item.key}
                        className="flex items-center justify-between gap-3 px-2.5 py-1.5 rounded-lg hover:bg-zinc-50 dark:hover:bg-zinc-800/50 transition-colors"
                      >
                        ...
                        <span
                          className={`text-[9px] px-2 py-0.5 rounded font-bold uppercase tracking-wider shrink-0 ${
                            isWrite
                              ...
                              : "bg-blue-100 dark:bg-blue-950/80 text-blue-800 dark:text-blue-300 border border-blue-200 dark:border-blue-800/60"
                          }`}
                        >
                          {item.level}
                        </span>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-zinc-200 dark:border-zinc-800 bg-slate-50 dark:bg-[#18181b]/60">
                  ...
                  <th className="px-6 py-3.5 text-[11px] font-semibold text-zinc-600 dark:text-zinc-400 uppercase tracking-wider min-w-[280px]">Assigned Permissions</th>
                  ...
                  return (
                    <tr key={member.id} className="hover:bg-slate-50/80 dark:hover:bg-zinc-800/30 transition-colors group">
                      ...
                      <td className="px-6 py-4 min-w-[280px]">
                        {isAdmin ? (
                          <span className="inline-flex items-center gap-1.5 text-xs text-purple-700 dark:text-purple-300 font-semibold bg-purple-50 dark:bg-purple-950/40 px-2.5 py-1 rounded-lg border border-purple-200 dark:border-purple-900/50 shadow-2xs whitespace-nowrap">
                          ...
                          <span className="text-xs text-zinc-400 dark:text-zinc-600 italic whitespace-nowrap">No sections assigned</span>
                        ) : grantedSections.length === ASSIGNABLE_SECTION_KEYS.length && grantedSections.every((s) => s.level === "write") ? (
                          <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-semibold bg-emerald-50 dark:bg-emerald-950/40 text-emerald-700 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-800/60 shadow-2xs whitespace-nowrap">
                            ...
                            All Sections (Full Control)
                          </span>
                        ) : grantedSections.length === ASSIGNABLE_SECTION_KEYS.length && grantedSections.every((s) => s.level === "read") ? (
                          <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-semibold bg-blue-50 dark:bg-blue-950/40 text-blue-700 dark:text-blue-300 border border-blue-200 dark:border-blue-800/60 shadow-2xs whitespace-nowrap">
