            ...
            {pagination.totalPages > 1 && (
              <div className="flex items-center justify-between px-6 py-4 border-t border-zinc-200 dark:border-zinc-800">
                ...
                <div className="flex items-center gap-2">
                  <button onClick={() => setPage(Math.max(1, page - 1))} disabled={page === 1} className="px-3 py-1.5 text-xs text-zinc-400 hover:text-white bg-zinc-900 hover:bg-zinc-800 border border-zinc-800 disabled:opacity-40 disabled:cursor-not-allowed rounded-lg transition cursor-pointer">Previous</button>
                  {Array.from({ length: pagination.totalPages }, (_, i) => i + 1).filter((p) => p === 1 || p === pagination.totalPages || Math.abs(p - page) <= 1).reduce<(number | "ellipsis")[]>((acc, p, i, arr) => { if (i > 0 && p - (arr[i - 1] as number) > 1) acc.push("ellipsis"); acc.push(p); return acc; }, []).map((item, i) => item === "ellipsis" ? (<span key={`e${i}`} className="px-2 text-zinc-600">...</span>) : (<button key={item} onClick={() => setPage(item)} className={`w-8 h-8 text-xs font-semibold rounded-lg transition cursor-pointer ${page === item ? "bg-zinc-800 text-zinc-100 border border-zinc-700/50 shadow-xs" : "text-zinc-400 hover:text-white bg-zinc-900 hover:bg-zinc-800 border border-zinc-800"}`}>{item}</button>))}
                  <button onClick={() => setPage(Math.min(pagination.totalPages, page + 1))} disabled={page === pagination.totalPages} className="px-3 py-1.5 text-xs text-zinc-400 hover:text-white bg-zinc-900 hover:bg-zinc-800 border border-zinc-800 disabled:opacity-40 disabled:cursor-not-allowed rounded-lg transition cursor-pointer">Next</button>
                </div>
              </div>
            )}
          </>
