...
export default function EditTabModal({ tab, onClose, onSuccess }: { tab: LeadTab; onClose: () => void; onSuccess: (tab: LeadTab) => void }) {
  ...
  async function handleSave() {
    ...
    setSaving(true);
    try {
      const res = await fetch(`/api/admin/leads/tabs/${tab.id}`, {
        method: "PUT",
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80">
      <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl w-full max-w-sm mx-4 shadow-2xl">
        <div className="flex items-center justify-between px-6 py-5 border-b border-zinc-200 dark:border-zinc-800">
          <h2 className="text-lg font-semibold text-zinc-900 dark:text-white">Edit Tab</h2>
