  ...
  async function handleCreate() {
    ...
    try {
      const res = await fetch("/api/admin/leads/files", {
        ...
        body: JSON.stringify({ name, description: description || null, folderId: selectedFolderId || null }),
      });
        ...
        <div className="px-6 py-5 space-y-4">
          ...
          {flatFolders.length > 0 && (
            <div>
              ...
              <select value={selectedFolderId} onChange={(e) => setSelectedFolderId(e.target.value)} className="w-full bg-slate-50 dark:bg-zinc-800/50 border border-zinc-200 dark:border-zinc-800 rounded-xl px-4 py-2.5 text-sm text-zinc-900 dark:text-zinc-300 focus:outline-none focus:ring-2 focus:ring-blue-500/20 dark:focus:ring-zinc-700 transition cursor-pointer">
