...
export default function EditColumnModal({
  ...
  const [type, setType] = useState<"text" | "select" | "number">((column.type as "text" | "select" | "number") || "text");
  const [options, setOptions] = useState(column.options || "Pending, In Progress, Done, Verified");
  ...
  async function handleUpdate() {
    ...
    setSaving(true);
    try {
      const res = await fetch(`/api/admin/leads/columns/${column.id}`, {
        method: "PUT",
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4">
      <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl w-full max-w-md shadow-2xl animate-in fade-in zoom-in-95 duration-100">
        <div className="flex items-center justify-between px-6 py-5 border-b border-zinc-200 dark:border-zinc-800">
          <div>
            <h2 className="text-lg font-semibold text-zinc-900 dark:text-white">Edit Column & Options</h2>
            <p className="text-xs text-zinc-500 dark:text-zinc-400 mt-0.5">Customize column name, type, and dropdown options</p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="px-6 py-5 space-y-4">
          <div>
            ...
            <input
              ...
              onKeyDown={(e) => e.key === "Enter" && type !== "select" && handleUpdate()}
              autoFocus
              placeholder="e.g. Status, Priority, Action"
            ...
            <div className="space-y-2 bg-slate-50 dark:bg-[#18181b] border border-zinc-200 dark:border-zinc-800 rounded-xl p-3.5">
              <div className="flex items-center justify-between">
                <label className="text-zinc-800 dark:text-zinc-300 text-xs font-medium">
                  Dropdown Menu Options <span className="text-zinc-500 font-normal">(comma-separated)</span>
                </label>
